...
impl Api {
    ...
    fn setup(&self, url: &str, bytes: &[u8]) -> Result<(), String> {
        let body = if self.v3 {
            ...
            json!({ "url": url, "sha256": sha256_hex(bytes) })
        };
...
fn trimmed_mean(mut samples: Vec<f64>, trim: usize) -> f64 {
    ...
    let kept = if samples.len() > trim * 2 {
...
fn upload_script(agent: &Agent, url: &str, bytes: &[u8]) -> Result<String, String> {
    ...
    v.get("url")
        .and_then(Value::as_str)
        .map(str::to_string)
        .ok_or_else(|| "upload: no url in the reply".into())
}
...
mod tests {
    ...
    fn handle(stream: TcpStream, seen: Arc<Mutex<Vec<Req>>>, stop: Arc<AtomicBool>) {
        ...
        loop {
            ...
            let Some(body) = read_body(&mut reader, length, &stop) else {
                return;
            };
    ...
    fn v2_connect_identifies_sets_hssp_and_measures_the_clock() {
        let mock = Mock::start();
        let link =
            HandyLink::connect_to("KEY123", None, HandyHosting::Lan, mock.endpoints()).unwrap();
        assert_eq!(link.device, "Handy FW3 3.2.4");
        let paths = mock.paths();
        assert_eq!(paths[..3], ["connected", "info", "mode"]);
        assert_eq!(
            paths[3..].iter().filter(|p| *p == "servertime").count(),
            OFFSET_SAMPLES
        );
    ...
    fn old_firmware_and_an_unreachable_service_are_reported() {
        ...
        let ep = Endpoints {
            v2: "http://127.0.0.1:1/".into(),
            ..mock.endpoints()
        };
        assert!(HandyLink::connect_to("KEY", None, HandyHosting::Cloud, ep).is_err());
        assert_eq!(
            device_name(&json!({ "model": "Handy", "fwVersion": "4.0.1" })),
            "Handy FW4 4.0.1"
        );
    }
    ...
    fn an_error_body_beats_the_status() {
        let e = parse_body(
            "hssp/play",
            r#"{"error":{"code":1001,"message":"Device not connected"}}"#,
        )
        .unwrap_err();
    ...
    fn lan_hosting_sets_up_with_the_local_url_and_hash() {
        let mock = Mock::start();
        let mut link =
            HandyLink::connect_to("KEY", None, HandyHosting::Lan, mock.endpoints()).unwrap();
    ...
    fn play_stops_on_pause_and_restarts_after_a_seek() {
        ...
        assert_eq!(
            serde_json::from_str::<Value>(&mock.last("hssp/play").body).unwrap()["startTime"]
                .as_f64(),
            Some(60_000.0)
        );
    ...
    fn v3_takes_a_token_and_corrects_drift_with_synctime() {
        ...
        assert_eq!(
            (
                ...
                play["playback_rate"].as_f64(),
                play["loop"].as_bool()
            ),
            (Some(1_000.0), Some(1.0), Some(false))
        );
