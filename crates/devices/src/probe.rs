...
pub fn probe_ports(paths: &[String], wait: Duration) -> Vec<ProbedPort> {
    let handles: Vec<_> = paths
        ...
        .collect();
    handles.into_iter().map(|h| h.join().unwrap_or_else(|_| ProbedPort { path: String::new(), device: None, tcode: None, error: Some("probe thread died".into()) })).collect()
}
...
pub(crate) fn is_identity(line: &str) -> bool {
    ...
    l.chars().count() >= 3
        ...
        && l.chars().all(|c| !c.is_control() && (c.is_alphanumeric() || c.is_ascii_punctuation() || c == ' '))
}
...
fn probe_port(path: &str, wait: Duration) -> ProbedPort {
    ...
    let mut port = match serialport::new(path, 115_200).timeout(Duration::from_millis(50)).open() {
        Ok(p) => p,
        Err(e) => {
...
mod tests {
    ...
    #[test]
    fn identity_lines() {
        assert!(is_identity("SR6-OSR2-CraftyHandy-NTC v2.0 温控开启 (NTC34/LED32)"));
        assert!(!is_identity("`"));
