...
impl DeoFrame {
    pub fn identity(&self) -> Option<String> {
        self.resource.clone().or_else(|| self.path.clone()).filter(|s| !s.is_empty())
    }
...
fn whirligig_line(line: &str) -> Vec<FollowEvent> {
    ...
    match line.as_bytes().first() {
        Some(b'C') => {
            ...
            vec![FollowEvent::Path((!path.is_empty()).then(|| path.to_string()))]
        }
...
impl Follow {
    ...
    pub fn start(kind: FollowKind, host: &str, port: u16, sink: FollowSink) -> Follow {
        ...
        let spawned = {
            ...
            thread::Builder::new().name("bp-follow".into()).spawn(move || run(&inner, kind, &address))
        };
...
mod tests {
    ...
    fn splits_length_prefixed_frames() {
        ...
        assert_eq!(serde_json::from_slice::<DeoFrame>(&first).unwrap().identity().as_deref(), Some("a.mp4"));
        ...
        assert_eq!(serde_json::from_slice::<DeoFrame>(&payload).unwrap().current_time, Some(1.5));
    ...
    fn heresphere_prefers_resource_over_path() {
        let f: DeoFrame = serde_json::from_str(r#"{"path":"/dl/1","resource":"/v/a.mp4","playerState":1}"#).unwrap();
    ...
    fn follows_a_mock_deovr_server() {
        ...
        assert_eq!(events.iter().filter(|e| matches!(e, FollowEvent::Path(_))).count(), 1);
        assert_eq!(events.iter().filter(|e| matches!(e, FollowEvent::Duration(_))).count(), 1);
    }
}
