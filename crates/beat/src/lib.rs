...
//! that range, estimated again in overlapping eight-second windows. Repeated strong pulses
//! override the local timing for short fast sections and recurring subdivisions. Beats: the onset curve
//! tracked at the local tempo by dynamic programming, so a beat sits on an onset when there
//! is one and keeps time when there is not. Loudness: RMS in a short window around each beat,
//! against the track's peak. Strokes use the median audible beat as their depth reference.
...
/// Hop of the music model's grid, 50 Hz.
pub const GRID_HOP_MS: f64 = 20.0;

/// The analysis on the music model's 20 ms grid from time 0 (`ml/data/music_dataset.py`'s
/// `a2.npz`): onset strength, the phase within the beat as a sine and cosine pair, and the
/// loudness envelope. One implementation for training dumps and the app.
#[derive(Clone, Debug, Default, PartialEq)]
pub struct Grid50 {
    pub onset: Vec<f32>,
...
/// Value of a series sampled every `hop_ms` from `origin_ms`, linearly interpolated, held at
/// the ends, 0 when empty.
...
/// The track on the 50 Hz grid. The flux peaks when an onset reaches the middle of the STFT
/// window, so onset frame `i` is centred half a window in; an envelope step covers the hop it
/// was measured over; the phase runs 0..1 between consecutive beats, at the mean period before
/// the first and after the last, and is 0 with no beats at all.
...
/// Raw detections or speed-limited strokes built around them.
...
pub struct GenerateOptions {
    pub style: Style,
    /// Requested stroke depth, 0..2, capped by the range and speed.
    pub intensity: f64,
    /// Scale quiet strokes down relative to the track's typical audible beat.
    pub volume_depth: bool,
    /// 0.5 halves the tempo, 2 doubles it; combined axis speed supports 0.25 through 4.
    pub tempo_factor: f64,
    /// A rotation axis swings the other way on every other beat instead of ending low.
    pub alternate: bool,
    /// Add a bottom bounce on louder beats when there is room before the return peak.
    pub flourishes: bool,
    /// Fraction of the preceding stroke used by the bounce, 0.01..1.
    pub bounce_depth: f64,
    /// Bounce speed as a multiple of max_speed, 0.5..6.
    pub bounce_speed: f64,
    pub fps: f64,
    /// Position units per second, before playback-rate compensation.
    pub min_speed: f64,
    /// Main stroke speed; the bottom bounce multiplies this by bounce_speed.
    pub max_speed: f64,
...
/// Reuse the preceding spectrum across chunks. Frequency weighting reduces rumble and
/// gives low percussion more influence without discarding higher percussion.
struct SpectralFlux {
    window: Vec<f64>,
    previous: Vec<f64>,
    magnitudes: Vec<f64>,
...
/// Power (squared magnitude) of the first `out.len()` bins of `input`'s spectrum, radix-2.
/// `input` must be a power of two long. Shared with the model crate's log-mel.
...
fn tempo(onsets: &[f64], hop_ms: f64, previous: Option<f64>) -> f64 {
    ...
    // Random flux can have a strongest lag without a repeating beat. Keep the preceding
    // estimate until a window has enough periodic evidence to replace it.
...
/// Estimate every second over eight seconds, then follow repeated strong onset gaps without
/// smoothing their transitions. Silence keeps the preceding estimate. `bpm` remains the
/// whole-track summary used by the UI and model.
...
/// Repeated strong pulses can change pace inside the tempo window. Use their actual gaps,
/// including recurring subdivisions, while retaining the tempo estimate through weak sound.
fn follow_strong_onsets(onsets: &[f64], hop_ms: f64, periods: &mut [f64]) {
    // An onset can straddle FFT frames. Its nearby energy is more stable across audio
    // window offsets than the height of its strongest frame.
    ...
    // A tempo change can begin between beats. Join its two confirmed runs using the actual
    // boundary gap; an isolated offbeat has two unconfirmed gaps and cannot pass this check.
...
/// Beat frames at the local period, by dynamic programming over the onset curve: each
...
/// Continues the beats past the last one at the recent tempo until `ahead_ms` after it, so
/// a live track has beats to play before the next analysis. The added beats take the mean
/// loudness of the last four (1 with none). Nothing with fewer than two beats or no tempo.
...
mod tests {
    ...
    fn the_grid_samples_onset_envelope_and_phase_as_the_dumper_did() {
        // Two beats 500 ms apart, an onset curve that ramps, an envelope that steps.
        let track = BeatTrack {
        ...
        // Onset frame 0 sits at 23.2 ms; before it the first value holds, after it the ramp is
        // the time past the origin in hops.
        ...
        // The envelope's first step is centred 50 ms in, so 50 ms reads 0 and 150 ms reads 1.
        assert_eq!(g.loudness[0], 0.0);
        assert!((g.loudness[5] - 0.5).abs() < 1e-6);
        assert_eq!(g.loudness[59], 1.0);
        // Phase: 0 on a beat, 0.5 midway, negative before the first at the mean period.
