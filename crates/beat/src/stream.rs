...
/// A bounded audio window whose FFT frames are computed once, including across chunk edges.
#[derive(Default)]
pub struct Stream {
    samples: Vec<f32>,
    spectra: Vec<(f64, f64)>,
...
impl Stream {
    ...
    /// Return absolute beat times and the most recent beat supported by audible onset energy.
    /// The caller may predict briefly from that evidence, then stop when it becomes stale.
...
mod tests {
    ...
    fn rolling_audio_deadline_and_timing_recovery() {
        ...
        // Release builds are used for timing; debug instrumentation is intentionally slower.
