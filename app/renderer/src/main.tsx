...
import { Recover, RecoverScreen } from './app/Recover'
...
async function boot() {
  ...
  const report = (where: string, error: unknown) => void invoke('app:reportError', `${where}: ${error instanceof Error ? (error.stack ?? error.message) : String(error)}`)
  window.addEventListener('error', (e) => report('uncaught', e.error instanceof Error ? e.error : e.message))
  ...
  void startI18n().catch((error: unknown) => report('i18n', error))
  ...
  try {
    ...
    report('boot', error)
    flushSync(() => reactRoot.render(<RecoverScreen />))
  }
}
